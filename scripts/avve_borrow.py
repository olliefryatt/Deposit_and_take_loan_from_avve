...
def main():
    ...
    print(f'Amount of WETH want to deposit: {amount}')
    # Change ETH to wETH if necessary
    if network.show_active() in ["mainnet-fork"]:
        print("getting wETH")
        get_weth()
    # Get lending pool address
    lending_pool = get_lending_pool()
    print(f'Lending pool to deposit into: {lending_pool}')
...
def get_asset_price(price_feed_address):
    ...
    print(f"The ETH/DAI price is {1 / converted_latest_price}")
...
def get_borrowable_data(lending_pool, account):
    (
        ...
        ltv,
        health_factor,
    ) = lending_pool.getUserAccountData(account.address) # Is view function so no gas
